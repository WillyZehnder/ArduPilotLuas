...
-- Version 28.02.2024: Rotation by wind_estimate
-- Version 29.02.2024: send_msg calls optimized
-- Version 01.03.2024: send_msg calls "Wind..." optimized
local SCRIPT_VERSION        = '01.03.2024'
...
local rotation              = 0.0             -- rotation of mission if selected
...
local function get_wind_direction()
  ...
  if wind then
    ...
    local windspeed = wind_xy:length()
    local winddeg = math.fmod(180+math.deg(wind_xy:angle()), 360)
    -- send_msg(INFO, string.format('Wind %4.1f m/s %4.0f deg', windspeed, winddeg))
    local windrad = math.fmod(wind_xy:angle()+math.pi, math.pi*2)
    return windrad
  else
    send_msg(WARN, 'No wind_estimation available')
    return 0.0
  end
end
...
--    rotation = assert(ahrs:get_yaw(), 'no yaw available') -- memorize current heading
      rotation = get_wind_direction()
      send_msg(INFO, string.format('translation & rotation %.0f deg', math.deg(rotation)) )
